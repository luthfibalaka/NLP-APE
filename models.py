...
from bmes_decode import extract_span_arguments_yi
...
class BERT_BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.longtokenizer = LongformerTokenizer.from_pretrained("./longformer-base/")
        self.longtokenizer.add_special_tokens(self.special_tokens_dict_yi)

        self.longformerconfig = LongformerConfig.from_pretrained("./longformer-base/")
        ...
        self.am_bilstm = nn.LSTM(
            config.bert_output_size,
            config.hidden_size,
            num_layers=1,
            bidirectional=config.is_bi,
            batch_first=True,
        )
        ...
        self.span_embedding = MultiNonLinearClassifier(
            config.hidden_size * 2, 1, config.mrc_dropout, intermediate_hidden_size=128
        )  # 256
    ...
    def bert_emb_for_task1(self, para_tokens_list):
        ...
        question_tokens = "[q]"
        question_tokens_cls_sep = (
            self.longtokenizer.cls_token
            + " "
            + question_tokens
            + " "
            + self.longtokenizer.sep_token
        )
        ...
        sent_length_list = [
            len(sent.split(" ")) for para in para_tokens_list for sent in para
        ]
        passage_tokens = " ".join(sent_tokens_list)
        passage_tokens_cls_sep = (
            self.longtokenizer.cls_token
            + " "
            + passage_tokens
            + " "
    ...
    def bert_emb_for_task2(self, para_tokens_list, argument_para_tokens_list):
        ...
        pair_ids_padding_list, pair_mask_list, max_len = (
            self.padding_and_mask_with_return(pair_ids)
        )
    ...
    def am_tagging_span_for_task2(
        self,
        rev_para_tokens_list,
        rep_para_tokens_list,
        arg_pair_sems_list,
        mode="train",
    ):
        ...
        for batch_i, pred_arguments_labeldict in enumerate(arg_pair_sems_list):
            for rev_arg_span, label_dict in pred_arguments_labeldict.items():
                ...
                temp_argu_o = rev_para_tokens_list[batch_i][
                    rev_arg_span[0] : rev_arg_span[1] + 1
                ]
    ...
    def am_tagging_with_task1_for_task2(
        self,
        rev_para_tokens_list,
        rep_para_tokens_list,
        arg_list_from_task1,
        ...
        for batch_i, pred_arguments in enumerate(arg_list_from_task1):
            for rev_arg_span in pred_arguments:
                temp_argu_o = rev_para_tokens_list[0][
                    rev_arg_span[0] : rev_arg_span[1] + 1
                ]
                temp_arg_list.append(temp_argu_o)
    ...
    def forward(
        ...
        for (
            ...
            if tag == "task1_review" or tag == "task1_reply":
                ...
                # review
                start_logits, end_logits, span_logits = self.am_tagging_span_for_task2(
                    para_tokens_list,
            ...
            start_loss, end_loss, match_loss = self.compute_loss(
                ...
                end_logits,
                span_logits,
                start_labels.expand(1, -1).cuda(),
                end_labels.expand(1, -1).cuda(),
    ...
    def predict_span(
        ...
        review_start_logits, review_end_logits, review_span_logits = (
            self.am_tagging_span_for_task1(review_para_tokens_list, mode="test")
        )
        review_start_preds, review_end_preds, review_span_preds = (
            F.sigmoid(review_start_logits) > 0.5,
            F.sigmoid(review_end_logits) > 0.5,
            F.sigmoid(review_span_logits) > 0.5,
        )
        ...
        # reply
        reply_start_logits, reply_end_logits, reply_span_logits = (
            self.am_tagging_span_for_task1(reply_para_tokens_list, mode="test")
        )

        reply_start_preds, reply_end_preds, reply_span_preds = (
            F.sigmoid(reply_start_logits) > 0.5,
            F.sigmoid(reply_end_logits) > 0.5,
            F.sigmoid(reply_span_logits) > 0.5,
        )
        ...
        pred_rev_args_list = extract_span_arguments_yi(
            pred_rev_args_dict["review_span_preds"],
            pred_rev_args_dict["review_start_preds"],
            pred_rev_args_dict["review_end_preds"],
        )
        pred_rep_args_list = extract_span_arguments_yi(
            pred_rep_args_dict["reply_span_preds"],
        ...
        if test_rev_args_list == []:
            ...
            try:
                # review
                review_start_logits, review_end_logits, review_span_logits = (
                    self.am_tagging_with_task1_for_task2(
                        review_para_tokens_list,
                        reply_para_tokens_list,
                        pred_rev_args_list,
                        mode="test",
            ...
            pred_pair_rep_args_list = extract_span_arguments_yi(
                review_span_preds_list, review_start_preds_list, review_end_preds_list
            )
            ...
            for true_arguments in pred_rev_args_list:
                ...
                for args in true_arguments:
                    pred_args_pair_dict[args] = (
                        pred_pair_rep_args_list[i],
        ...
        if test_rep_args_list == []:
            ...
            pred_pair_rev_args_list = extract_span_arguments_yi(
                rebuttal_span_preds_list,
                rebuttal_start_preds_list,
