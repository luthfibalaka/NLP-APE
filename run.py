...
from bmes_decode import (
    get_arg_span,
    spans_to_tags,
    extract_span_arguments_yi,
)
    ...
    with open(file_path, "r") as fp:
        ...
        for rr_pair in rr_pair_list:
            ...
            start_labels = torch.LongTensor(
                [(1 if idx in review_start_positions else 0) for idx in range(seq_len)]
            )
            end_labels = torch.LongTensor(
                [(1 if idx in review_end_positions else 0) for idx in range(seq_len)]
            )
            ...
            start_labels = torch.LongTensor(
                [(1 if idx in reply_start_positions else 0) for idx in range(seq_len)]
            )
            end_labels = torch.LongTensor(
                [(1 if idx in reply_end_positions else 0) for idx in range(seq_len)]
            )
            ...
            for rev_arg_span in sample_review["arg_spans"]:
                rev_arg_pair_id = int(
                    sample_review["pair_tags"][rev_arg_span[0]].split("-")[-1]
                )
                rev_arg_2_rep_arg_dict[rev_arg_span] = []
                for rep_arg_span in sample_reply["arg_spans"]:
                    rep_arg_pair_id = int(
                        sample_reply["pair_tags"][rep_arg_span[0]].split("-")[-1]
                    )
            ...
            for rev_arg_span, rep_arg_spans in rev_arg_2_rep_arg_dict.items():
                ...
                pair_match_labels = torch.zeros(
                    [rep_seq_len, rep_seq_len], dtype=torch.long
                )
                ...
                pair_start_labels = torch.LongTensor(
                    [
                        (1 if idx in pair_reply_start_positions else 0)
                        for idx in range(rep_seq_len)
                    ]
                )
                pair_end_labels = torch.LongTensor(
                    [
                        (1 if idx in pair_reply_end_positions else 0)
                        for idx in range(rep_seq_len)
                    ]
                )
            ...
            for rep_arg_span, rev_arg_spans in rep_arg_2_rev_arg_dict.items():
                ...
                pair_match_labels = torch.zeros(
                    [rev_seq_len, rev_seq_len], dtype=torch.long
                )
                ...
                pair_start_labels = torch.LongTensor(
                    [
                        (1 if idx in pair_review_start_positions else 0)
                        for idx in range(rev_seq_len)
                    ]
                )
                pair_end_labels = torch.LongTensor(
                    [
                        (1 if idx in pair_review_end_positions else 0)
                        for idx in range(rev_seq_len)
                    ]
                )
...
def load_data(file_path):
    ...
    with open(file_path, "r") as fp:
        ...
        for rr_pair in rr_pair_list:
            ...
            sample_list.append({"review": sample_review, "reply": sample_reply})
    return sample_list
...
def evaluate(model, data_list):
    ...
    for batch_i in tqdm(range(data_len)):
        ...
        for sample in data_batch:
            ...
            rep_arg_2_rev_arg_sems_list.append(
                sample["reply"]["rep_arg_2_rev_arg_dict_sem"]
            )
            ...
            rev_arg_2_rep_arg_sems_list.append(
                sample["review"]["rev_arg_2_rep_arg_dict_sem"]
            )
        ...
        true_rev_args_list_span = extract_span_arguments_yi(
            review_match_labels, review_start_labels, review_end_labels
        )
        all_true_rev_args_list.extend(true_rev_args_list_span)
        pred_rev_args_list_span = extract_span_arguments_yi(
        ...
        true_rep_args_list_span = extract_span_arguments_yi(
            reply_match_labels, reply_start_labels, reply_end_labels
        )
        all_true_rep_args_list.extend(true_rep_args_list_span)
        pred_rep_args_list_span = extract_span_arguments_yi(
...
(
    train_list_task1_review,
    train_list_task1_reply,
    train_list_task2_review,
    train_list_task2_reply,
...
other_model_para = list(
    set(model.parameters()) - set(longformer_model_para) - set(lstm_para)
)
...
optimizer_grouped_parameters = [
    {
        "params": [p for p in other_model_para if len(p.data.size()) > 1],
...
for epoch_i in range(config.epochs):
    ...
    for batch_i in tqdm(range(train_iter_len)):
        ...
        if True:
            ...
            model.train()
            train_batch = train_list[
                batch_i * config.batch_size : (batch_i + 1) * config.batch_size
            ]
    ...
    if total_f1 > best_f1:
        ...
        logger.warning("total pair f1:\t{:.4f}".format(dev_args_pair_dict["f1"]))

        test_args_pair_dict = evaluate(model, test_list)
        logger.warning("*" * 20 + "the performance in test set..." + "*" * 20)
        logger.warning("total pair f1:\t{:.4f}".format(test_args_pair_dict["f1"]))
