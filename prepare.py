...
for data_file in ["dev.txt", "test.txt", "train.txt"]:
    with open(root_dir + data_file, "r", encoding="utf-8") as fp:
        raw_sample_list = fp.read().split("\n\n")
        print(data_file + ":" + str(len(raw_sample_list)))

    with open(processed_root_dir + data_file, "w", encoding="utf-8") as fp:
        sample_dict_list = []
        ...
        for sample_dict in sample_dict_list:
            ...
            for token_sent, bio_tag, pair_tag in zip(
                review_dict["sents"], review_dict["bio_tag"], review_dict["pair_tag"]
            ):
            ...
            for token_sent, bio_tag, pair_tag in zip(
                reply_dict["sents"], reply_dict["bio_tag"], reply_dict["pair_tag"]
            ):
                token_sent = preprocess(token_sent)
                fp.write(
                    "{}\t{}\t{}\t{}\t{}\n".format(
